...
class FileRepository {
    ...
    public function upload( $form_data, $type ) {
        $validator = Validator::make($form_data, \App\Model\File::$rules,\App\Model\File::$messages);
        if ($validator->fails()) {
    ...
    public function delete($filename) {
        $sessionImage =  \App\Model\File::where('file_name', 'like', $filename)->first();

        if(empty($sessionImage))
        {
            $sessionImage  =  \App\Model\File::where('display_name', 'like', $filename)->first();
            if(empty($sessionImage)) {
                return Response::json([
                    'error' => true,
                    'file_name' => $filename,
